...
(defn config-setup! []
  (let [tmp-dir (System/getProperty "java.io.tmpdir")]
    (swap! shared/conf (fn [orig-conf]
                       (conj orig-conf 
                             {:working-dir (create-tmp-dir "domina-executer-test-working-dir_")
                              :git-repos-dir (create-tmp-dir "domina-executer-git-repo-dir_")
                              :tmp-dir (create-tmp-dir "domina-executer-tmp-dir_") 
...
(deftest git
  (testing "prepare-and-create-working-dir"
    (config-setup!)
    (unpack-test-repo!)
    (let [new-params (prepare-and-create-working-dir {:git_url (:test-repo-path @shared/conf) 
                                                      :git_repository_id  "test_repo"
      ...
      (testing "it includes a :working_dir key in the result params"
        (is (not= nil (:working_dir new-params))))

      (testing "the working dir is checked out git repo with a commit present"
        (let [res (shell/sh "git" "log" "-n" "1" "--format='%H'" "e4e1e98473b51b5539a16741da717f4e2ae33965" 
                            :dir (:working_dir new-params))]
          (is (= 0 (:exit res))))
        (is (= true (.exists (File.  (str (:working_dir new-params)  (File/separator) "README.mdk"))))) )
      )))
